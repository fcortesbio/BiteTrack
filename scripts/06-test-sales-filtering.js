...
class SalesFilteringTester {
  ...
  async testPagination() {
    ...
    this.log(`  Pagination working: ${page1.pagination.totalSales} total sales across ${page1.pagination.totalPages} pages`);
  }

  async testSorting() {
    // Test ascending sort by total amount
    const ascending = await this.apiCall('/sales', { sort: 'totalAmount', limit: 5 });
    ...
    const descending = await this.apiCall('/sales', { sort: '-totalAmount', limit: 5 });
  ...
  async testSettledFiltering() {
    ...
    this.log(`  Settlement filtering: ${settled.sales.length} settled, ${unsettled.sales.length} unsettled`);
  }
  ...
  async testDateFiltering() {
    ...
    if (filtered.sales.length !== allSales.sales.length) {
      ...
      this.log(`  Date filter returned ${filtered.sales.length} vs ${allSales.sales.length} total (acceptable variance)`);
    }
  ...
  async testPopulatedReferences() {
    ...
    this.log(`  References populated: Customer "${sale.customerId.firstName}", Product "${product.productId.name}"`);
  }
  ...
  async testCombinedFiltering() {
    ...
    for (const sale of combined.sales) {
      if (sale.settled) {
        throw new Error('Combined filter returned settled sale when filtering for unsettled');
      }
    }
    ...
    this.log(`  Combined filtering: ${combined.sales.length} unsettled sales, sorted by creation date`);
  }
  ...
  async runAllTests() {
    ...
    await this.runTest('Basic Sales List Structure', () => this.testBasicSalesList());
    await this.runTest('Pagination Functionality', () => this.testPagination());
    await this.runTest('Sorting Functionality', () => this.testSorting());
    await this.runTest('Settlement Status Filtering', () => this.testSettledFiltering());
    await this.runTest('Date Range Filtering', () => this.testDateFiltering());
    await this.runTest('Populated References', () => this.testPopulatedReferences());
    await this.runTest('Combined Filtering', () => this.testCombinedFiltering());
