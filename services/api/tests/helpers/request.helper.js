...
const requestValidators = {
   ...
   * @returns {Promise<Object>} Response object
   */
  testAuthorizationRequired: async (
    app,
    method,
    endpoint,
    token,
...
const commonTestScenarios = {
  ...
  paginationTests: (app, endpoint, token = null, _testData = {}) => ({
    ...
    testCustomPageSize: async (limit = 5) => {
      const req = token
        ? requestBuilders.authenticatedGet(app, endpoint, token, {
            page: 1,
            limit,
          })
    ...
    testInvalidPagination: async () => {
      const req = token
        ...
        : requestBuilders.unauthenticatedGet(app, endpoint, {
            page: -1,
            limit: 0,
          });
  ...
  crudTests: (app, baseEndpoint, token, testData) => ({
    ...
    testCreate: async (data = testData.valid) => {
      return await requestBuilders.authenticatedPost(
        app,
        baseEndpoint,
        data,
        token,
    ...
    testGetById: async (resourceId) => {
      return await requestBuilders.authenticatedGet(
        app,
        `${baseEndpoint}/${resourceId}`,
        token,
      );
    },
    ...
    testUpdate: async (resourceId, updateData = testData.update) => {
      return await requestBuilders.authenticatedPut(
        app,
        `${baseEndpoint}/${resourceId}`,
        updateData,
        token,
    ...
    testDelete: async (resourceId) => {
      return await requestBuilders.authenticatedDelete(
        app,
        `${baseEndpoint}/${resourceId}`,
        token,
      );
