  ...
  describe('POST /bitetrack/auth/reset', () => {
    describe('Success scenarios', () => {
      it('should reset password with valid token and seller details', async () => {
        ...
        const isNewPassword = await bcrypt.compare('NewPassword123!', updatedSeller.password);
        expect(isNewPassword).toBe(true);

        // Verify token was deleted
        const deletedToken = await PasswordResetToken.findOne({ token: 'valid-reset-token' });
