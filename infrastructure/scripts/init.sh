...
main() {
    ...
    if [[ "$SETUP_MODE" == "dev" ]]; then
        ...
        echo "  - MongoDB container running on localhost:27017"
        echo "  - Ready for local development AND production deployment"
        echo ""
        echo -e "${BLUE}Next Steps:${NC}"
        echo "  For development:"
        echo "    - Run: npm run dev (services on ports 3001, 4001, 5001)"
        echo ""
        echo "  For production:"
        echo "    - Run: docker compose up -d (starts full stack)"
        echo "    - Access: http://localhost/bitetrack/health"
    fi
    echo ""
